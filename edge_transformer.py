...
class Composer(torch.nn.Module):
    def __init__(
        ...
        self.signet = signet
        if self.signet:
            self.signet_proj = MLP(concat_dim, embed_dim, linear=linear)
    ...
    def forward(self, x, edge_index, edge_attr, batch, token_index, token_attr=None, signet_out = None):
        ...
        if signet_out is not None:
            signet_out = signet_out[token_index.T].flatten(1, 2)
            signet_out = self.signet_proj(signet_out)
            signet_out = to_dense_adj(token_index, batch, signet_out)
...
class EdgeTransformer(torch.nn.Module):
    def __init__(
        ...
        has_edge_attr=False,
        compiled: bool = False,
        signet_presence = False
    ):
        super().__init__()
        self.feature_encoder = feature_encoder
        self.composer = Composer(embed_dim, has_edge_attr, signet=signet_presence)
        ...
        self.signet_presence = signet_presence
        # if self.signet_presence:
        #     self.linear_concat = torch.nn.Linear(2*embed_dim, embed_dim)
    ...
    def forward(self, data, pos = None):
        ...
        x = self.composer(
            data.x, data.edge_index, data.edge_attr, data.batch, token_index, token_attr, signet_out = pos
        )
...
class SignNetEdgeTransformer(torch.nn.Module):
    def __init__(   self, 
                    feature_encoder,
                    num_layers,
                    embed_dim,
                    ...
                    activation,
                    signet_hidden_dim, 
                    signet_num_phi_layers, 
                    signet_num_rho_layers, 
                    pooling=None,
                    attention_dropout=0.0,
                    ffn_dropout=0.0,
                    head_project_down=True,
        ...
        super().__init__()
        self.sign_net = SignNet(signet_hidden_dim, signet_num_phi_layers, signet_num_rho_layers, ignore_eigval=ignore_eigval)
       
        self.edge_transformer = EdgeTransformer(feature_encoder,
                                                num_layers,
                                                embed_dim,
                                                out_dim,
                                                num_heads,
                                                ...
                                                compiled = compiled,
                                                signet_presence = True)

        self.signet_pe = signet_pe
    ...
    def forward(self, data):
        ...
        return self.edge_transformer(data, pos)
